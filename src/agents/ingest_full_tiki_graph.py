...
class TikiEnterpriseGraph:
    ...
    def ingest_relationships(self, folder):
        ...
        # offer_coupon -> (Product/Price) -[:AP_DUNG_MA]-> (Coupon)
        ...
        # offer_service -> (Product/Price) -[:KEM_DICH_VU]-> (Service)
    ...
    def _merge_nodes(self, folder, file_name, label, id_col, set_properties, batch_size=10000):
        ...
        if df is not None:
            ...
            for i in range(0, total, batch_size):
                batch = records[i:i + batch_size]
                self.run_cypher(query, {"rows": batch})
                print(f"+ Creating batch: {i} -> {i + len(batch)} / {total}")
