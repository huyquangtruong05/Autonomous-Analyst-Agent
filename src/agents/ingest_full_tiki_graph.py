...
# --- THIẾT LẬP ĐƯỜNG DẪN VÀ BIẾN MÔI TRƯỜNG ---
BASE_DIR = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
env_path = os.path.join(BASE_DIR, '.env')
load_dotenv(dotenv_path=env_path)
...
class TikiEnterpriseGraph:
    ...
    def prepare_database(self):
        print("[Neo4j] Đang dọn dẹp Database và tạo Constraint...")
        self.run_cypher("MATCH (n) DETACH DELETE n")
        nodes = ['Category', 'Coupon', 'Service', 'Seller', 'Reviewer', 'Product', 'Review', 'PriceOffer']
        for node in nodes:
            self.run_cypher(f"CREATE CONSTRAINT IF NOT EXISTS FOR (n:{node}) REQUIRE n.id IS UNIQUE")
            
        print("[Neo4j] Đang tạo Vector Index (384 dims) cho Review...")
        self.run_cypher("""
        CREATE VECTOR INDEX review_vector IF NOT EXISTS
    ...
    def ingest_nodes(self, folder):
        # 1. Các bảng Master Data (Nạp toàn bộ vì dung lượng nhẹ)
        self._merge_nodes(folder, 'category', 'Category', 'category_id', "n.name = row.category_name, n.level = toInteger(row.level)")
        self._merge_nodes(folder, 'seller', 'Seller', 'seller_id', "n.name = row.seller_name, n.rating = toFloat(row.seller_rating)")
        self._merge_nodes(folder, 'reviewer', 'Reviewer', 'reviewer_id', "n.name = row.reviewer_name")
        self._merge_nodes(folder, 'product', 'Product', 'product_id', "n.name = row.product_name, n.review_score = toFloat(row.review_score)")

        # 2. Bảng Review (CÓ LẤY MẪU & NHÚNG VECTOR)
        print("\n[HÀNH TRÌNH CHÍNH] Đang xử lý 5.000 Review bằng CPU...")
        self._merge_nodes(folder, 'review', 'Review', 'review_id', 
                          "n.content = row.review_content, n.rating = toFloat(row.rating_score)",
                          embed_col='review_content', batch_size=250, sample_size=5000)

    def _merge_nodes(self, folder, file_name, label, id_col, set_properties, batch_size=10000, embed_col=None, sample_size=None):
        df = self.load_parquet(folder, file_name)
        if df is not None:
            # --- CƠ CHẾ LẤY MẪU (SAMPLING) ---
            ...
            embeddings_model = None
            if embed_col:
                print(f"   -> Khởi tạo mô hình NLP Local cho nhãn {label}...")
                embeddings_model = HuggingFaceEmbeddings(model_name="paraphrase-multilingual-MiniLM-L12-v2")
                query = f"UNWIND $rows AS row MERGE (n:{label} {{id: toString(row.{id_col})}}) SET {set_properties}, n.embedding = row.embedding"
            else:
                query = f"UNWIND $rows AS row MERGE (n:{label} {{id: toString(row.{id_col})}}) SET {set_properties}"
            
            for i in range(0, total, batch_size):
                batch = records[i:i + batch_size]
                if embed_col and embeddings_model:
                    texts = [str(row.get(embed_col, "")) for row in batch]
                    vectors = embeddings_model.embed_documents(texts)
                    for j, row in enumerate(batch): row['embedding'] = vectors[j]

                self.run_cypher(query, {"rows": batch})
                print(f"   + Lô {label}: {i} -> {i + len(batch)} / {total}")

    def ingest_relationships(self, folder):
        print("\n[Neo4j] Đang thiết lập các mối quan hệ (Relationships)...")
        self._create_relation(folder, 'product', 'Product', 'product_id', 'Seller', 'seller_id', 'BAN')
        self._create_relation(folder, 'review', 'Review', 'review_id', 'Product', 'product_id', 'DANH_GIA_CHO')
        self._create_relation(folder, 'review', 'Reviewer', 'reviewer_id', 'Review', 'review_id', 'VIET')

    def _create_relation(self, folder, source_file, label1, key1, label2, key2, rel_name):
        df = self.load_parquet(folder, source_file)
        if df is not None:
            # Lấy mẫu quan hệ tương ứng nếu cần (để tránh lỗi MATCH không thấy node)
            records = df.to_dict('records')[:10000] 
            query = f"UNWIND $rows AS row MATCH (a:{label1} {{id: toString(row.{key1})}}) MATCH (b:{label2} {{id: toString(row.{key2})}}) MERGE (a)-[:{rel_name}]->(b)"
            self.run_cypher(query, {"rows": records})
            print(f"   + Quan hệ [:{rel_name}] hoàn tất.")
...
if __name__ == "__main__":
    ...
    graph_db.ingest_relationships(DATA_FOLDER)
    graph_db.close()
    print("\n✅ HOÀN TẤT NẠP DỮ LIỆU THỬ NGHIỆM!")